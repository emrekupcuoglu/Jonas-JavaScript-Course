...
//Watch the Lectures in the OOP section for more information on classes, objects, instances, inheritance, prototypes, and prototypal inheritance
...
const Person = function (firstName, birthYear) {
  ...
  //or even tens of thousands of person objects using this constructor function
  //If we did that then each of these objects would carry the calcAge function with them
  //So if we had a thorough objects we would essentially be creating thousand copies of the calcAge function
...
//Because we created the objects programmatically using the constructor function
...
console.log(Person.prototype.isPrototypeOf(Person)); //This returns false: that means Person.prototype is NOT the prototype of Person
...
//In step 3 __proto__ property is created and it sets its value to the prototype property of the function that is being called
//This is how JavaScript knows internally that the emre objects is connected to Person.prototype
...
//*However extending the prototype of a build in object is generally NOT a good idea
...
//First reason is that the next version of JavaScript might add a new method
...
//?ES6 Classes
//Classes in JavaScript do not work like in other languages
//Classes in JavaScript are basically syntactic sugars
//They still work the same, they still implement prototypal inheritance
//But with a syntax that makes more sense to people coming from other languages

//We can use class declarations or expressions when creating classes
//That is because classes are a special type of function
...
//*Class declaration
class PersonClass {
  //constructor works in a similar way to the constructor function
...
//Regular properties are called data properties
...
//This does not because from method is not on the prototype of Array
...
//With Object.create there is still the idea of prototypal inheritance however there is no prototype property involved
...
//creating the object like this is against the spirit of creating objects programmatically
...
const Student = function (firstName, birthYear, course) {
  ...
  // this.firstName = firstName;
  // this.birthYear = birthYear;
  //and if the implementation of Person changes in the future the changes won't be reflected here
...
class StudentClass extends PersonClass {
  constructor(fullName, birthYear, course) {
    ...
    //Idea is similar to what we did with the constructor function, but this time it happens automatically.
...
//it is a lot better to create methods that interact with these properties
...
//Let's create a deposit and withdrawal method inside the Account class above
...
//First it is to prevent a code from outside the class to accidentals manipulate the data inside the class
//That is the reason why we implemented the public interface(deposit, withdrawal, and requestLoan methods)
...
//Second reason: When w expose only a small interface, a small API consisting of only a couple public methods,
//then we can change all the other internal methods with more confidante
...
//We need to protect the movements so look there for more info
...
//*In this proposal there are 8 different kinds of class fields
...
class ClassFields {
  ...
  //*While Instance properties are more personalized because
  ...
  //If we try to read the #movements property the we'll get a syntax error(we have tried this below)
  #movements = [];
