...
//With exports we can export values out of a module e.g. simple values or even functions
...
//We can write code without modules but as the codebase gets bigger and bigger advantages of modules become more pronounced
...
//2. Isolating Components: Modules can be developed in isolation without thinking about the entire codebase,
//the engineer working on the module doesn't even need to understand all of it.
...
//and another on the screen and the best part of this each can work on their own module without understanding what the other engineers are doing and also
//without understanding how the entire final camera works itself

//3. Abstract Code: Implement low-level code in modules and import those abstractions to other modules.
...
//4. Organized Code: Modules naturally lead to a more organized codebase. Because when we break up our code into separate, isolated and, abstracted modules this will automatically organize our code make it easier to understand

//5. Re-usable Code:Modules allows us to easily reuse the same code even across multiple projects
//For example if we used a module to implement a couple of mathematical functions in a certain project, and if we then need the same function in a different project
...
//ES& modules are modules that are actually stored in files. Each file is one module exactly
...
//2. ES6 Modules are always run in strict mode but scripts are executed in "sloppy" mode by default
...
//5. In order to link a module to an HTML file we need to use the script tag with the type attribute set to module, we don't need this with regular scripts
...
//Modules are imported synchronously(but are still downloaded asynchronously these two are different things)
//Only after all imported modules are downloaded and executed the main script.js module will finally be executed
...
//If we were allowed to import a module inside of a function then that function would first have to be executed before the import could happen.
//And because of that modules wouldn't be able to be implemented in a synchronous way
...
//This is very important in large projects with hundreds of modules,
//and that includes 3rd party modules which we usually only want a small piece and not the entire module.
//By knowing all dependencies between modules bundlers like webpack and parcel can then join multiple modules together and eliminate dead code.
//So essentially this is the reason why we can only import and export outside of any code that needs to be executed like a function or an if block.
//After the parsing process has figured out which modules it needs to import then these modules are downloaded from the server (downloading happens in an asynchronous way)
//Only the importing operation itself happens synchronously
//After a module arrives it is parsed then the modules exports are linked to the imports in script.js
//e.g. math.js exports a function called rand and this function is linked to the rand import in the script.js module
...
//!This is quite important to understand because it is unique to ES6 modules, other module systems do NOT work like this but JavaScript modules do
//Next the code in the imported models are executed and with this process of importing modules is finally finished
//And now it is time for the importing module to be executed as well script.js in this example

//?Importing Module

//Exporting module is executed first we can check this by writing a console.log() in both the importing and the exporting module
//When we do this we will see that the exporting module's console.log() will execute first
//This is the syntax for named imports
//We can change the name of the imported variable using the as keyword
...
//?We can import everything with this syntax
//ShoppingCart is imported as an object
//If we think about it this module here is basically export a public API just like a class
...
//I just wanted to note that it looks similar
...
//We are importing the same module twice it doesn't cause any error but it is not advisable
...
//*Preferred style is to just use one default export per module and import that
...
//And if the cart was not a live connection we wouldn't see the changes
...
//?Top-Level Await
//Starting from ES2022 we can now use the await keyword outside of async functions at least in modules
...
//We can demonstrate this by logging something else afterwards
...
//*A More Real Example
//Many times we have situations where we have an async function that we want to return some data
...
//!The code below is commented out because parcel doesn't support top-level await
...
//Best way of achieving this is to simply use a function
//Because functions give us private data by default and allow us to return values which can become our public API

//We usually write an IIFE because this way we don't have to call it separately and we can also ensure it is only called once
//It is really important that this function is only created once
//Because the goal of this function NOT to re-use code by running it multiple times
//Only purpose of this function is to create a new scope and return data just once
...
//!!!!!!!!!IMPORTANT!!!!!!!!!!CLOSURES!!!!!!!!!!!
//How are we able to still manipulate the data indie the IIFE, if the IIFE has returned long ago
//Closures allow a function to have access to all the variables that were present at it's birthplace basically
//The reason addToCart function outside can still access the cart variable that was in the function
...
//Big consequence of this almost all of the modules in the NPM repository still use the CommonJS module system
//The reason for that the NPM was originally only intended for node.js which uses CommonJS
//Only later NPM became the standard repository for the whole JavaScript world and now we are basically stuck with CommonJS
//Therefore you will see a lot of CommonJS still around

//Just like ES6 modules in commonJS one file is one module

//Syntax for exporting in CommonJS
...
//!There are other modules systems and CommonJS is a particularly important one
...
//As a web developer it is important to know at least these basic commands using a terminal or command promo

//Because VS code has a terminal we''ll use it but you can use the command promo on windows or terminal on the mac
...
//We need to first specify the name of the file that will be moved and the location of the file that iw will be moved into
//To move it into the parent folder we do it like this: mv {file name} ../
//To move it inside a folder: mv {file name} {folder name}
...
//This deletes all the files and the directory itself recursively
...
//1. It doesn't make much sense having the HTMl loading all our JavaScript that is just messy
//2. Also many times we would usually download a library file into our computer directly for example: A JQuery JavaScript file
//But then whenever a new version come out we would have to
//manually go to the site, download the new version, change the file in our file system manually then included in the HTML again maybe with a different name because it can change name or the version number. That is a huge pain
...
//We can check if NPM is installed on our computer by writing npm -v to the terminal
//If we get a number the npm is installed. The number is the version number
...
//We can also install it using a shorter version
...
//2. We now have this folder called node_modules
...
//Because leaflet uses the CommonJS module system therefore we can not directly import it into our code
...
//We will NOT install the standard lodash
//Because that way it will only be usable with a module bundler
//Look for lodash-es on the lodash.com
//es stands for ES modules and use this command on the terminal:
//npm i lodash-es
//With this lodash is added to the package.json as a dependency
//We have one file for each of the methods available in lodash
...
//The parcel will automatically find the path to this module and will import it with us having to manually type the entire path
//!This works with all kinds of assets. Even with html, css, sass files, images, and of course with all sorts of modules
...
//Let's say that you want to move your project to another computer or share it with another developer or even check it into a version control like git
//In all of these scenarios you should never ever include the node modules folder
//When you coy your project to somewhere else there is no reason to include this huge node modules folder
//Because in a real project it will be really huge(you can have folders with 10000+ files)
//That will just slow you down and it doesn't make much sense either because all of these files are already at the NPM so you can always get them back from there
...
//Let's simulate losing the node_modules by deleting it
//All we have to do to fix this issue is write this into the terminal
//npm i
...
//Module bundler we are going to use is parcel it is super fast and easy to use and more importantly it works out of the box without any configuration
...
//Parcel is just another build tool that is also on the NPM
//We will use NPM to install it
//But this is a different dependency so we have to write --save-dev after the package name
//npm i parcel --save-dev
//A dev dependency is basically like a tool we need to build our application
...
//There are also global installations but more about that later
...
//Because parcel turns modules into regular scripts top-level await doesn't work
...
//Optimizations like white space removal and shortening variable names etc.
...
//dist folder is the folder we are going to send for production
...
//Whenever we save this file parcel will reload this file just like with live-server(this doesn't work in build versions because of all the optimizations)
//With parcel we can activate something event better which is called Hot module replacement.
//Hot Module Replacements:
//Whenever we change one of the modules it will trigger a rebuild.
//That new modified bundle will then automatically - like magic - get injected into the browser without triggering a whole page reload
//So whenever we change something in the code block below it will not reload that part of the page
//And that is amazing for maintaining state on our page whenever we are testing out something
//This used to be something quite annoying in the past.
//For example in our bankist application, whenever we re-loaded the page we needed to logging into the application
//But with parcel and hot module replacement that is not going to happen, because the page will not reload
//This code here is code that only parcel understands so it will not make it into our final bundle, because a browser is not going to understand any of it
if (module.hot) {
  module.hot.accept();
}
...
//?NPM Scripts
//There is a second way to run parcel and that is with using np scripts
...
//Then we run the script we crated using:
...
//We can add another script to the packlages.json file to build our project
...
//Parcel uses Babel to automatically transpile our code
//We can configure babel a lot if we want e.g. defining exactly which browsers should be supported
...
//Go to the documentation section and then to the plugins section
...
//That preset will automatically select which JavaScript features should be compiled based on browser support
...
//So if there are features that don't work go to the plugins and look at the Proposals section(it might have different name by hte time you need it)

//we will use the ES6 find method to find a product in a cart that has a quantity of 2 or greater
//If we could see this in parcel 2 we would see that the arrow function has been transpiled into a normal function
...
//!IMPORTANT
//Reason for this is that Babel can only transpile ES6 syntax
...
//It is easy to convert const and let to var, or an arrow function to a normal function
...
//and that is the async functions
...
//There are two fundamentally different ways of writing code in programing(paradigms)
...
//Code example of duplicating the value in an array
...
//then to increase that counter until we reach the length of the original array,
//then how exactly to store the new result in each new position of the array
//const arr=[2,4,6,8]
//const doubled=[];
...
//*the HOW(step-by-step instructions are abstracted away)
...
//In fact we have been using it all along without knowing that
...
//Which is the goal of using functional programming
//in the first place
//Functional programming is a huge paradigm that is
//which is really difficult to implement
//But it is still important to know these principles
//Many of the popular libraries such as react and redux
//are build around these principles
//In react state is also completely immutable
