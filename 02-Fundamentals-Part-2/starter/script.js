...
//with strict mode we can eliminate accidentel errors that would be headache inducing to solve
//because javascript would see no problem without strict mdoe
// with strict mode java script gives us helpful erros on what is wrong
//strict mode changes many things and it will be covered later when the time comes
//but it will be standart to use strict mode from now on
...
//about the wrong variable name hasDriversLicence is set to false initially 
...
//but because of a misspeling of hasDriversLicence !== hasDriverLicence (s is missing from Drivers)
//a new variable created at the spot and set to true instead of setting the original varibale to true
//with strict mode we can avoid mistakes like these
// let hasDriversLicence = false;
// const passedTest = true;


// if (passedTest) hasDriverLicence = true;
// if (hasDriversLicence) console.log("You can drive YAY!");
...
//code  between the paranthesis are called parameters
//parameters are spesific to the function and are defeined when the funciton is called
//code between the curly braces are called function body and is runned when the function is called
function logger() {
...
//fucntion above returned the value of juice
...
//this happpens beccause of a procces called hoisting
//
//we can't do this with function expressions

// const age1 = calcAge1(1998); we can call the funciton before declaring it
...
const calcAge2 = function (birthYear) { // this is an anonymous function because it doesn't have a name
  return 2037 - birthYear;
}
...
//we need to use paranthesis if there is more than one parameter
...
console.log(friends[friends.length - 1]); //we need to subtract 1 because arrrays start indexing at 0
//but length property returns the amount of items in the array which in this acse is 3
...
//only primative values are immutable arrays are NOT primative
...
//we can use differnt types in an array such as numbers and strings
...
//or JavaScript turns the whole array into a string and concatanes it 
...
// because we startted with an array it's a good idea to end it with one too
...
//Methods
//methods are technicaly functions but we called them on arrays
//this will be better explained later

// push method adds an element to the end of an array
const newLenght = friends.push("Ceren");
// push method returns the lengt of the element
...
//it returns the new lenght of the array as well
friends.unshift("Aziz");
console.log(friends);

//pop method is the opposite of push method
//pop method doesn't return the new lenght of the array
...
//simply returs true or false depending if the element is in the array or not
...
//we can only acces the data they hold with the index number
...
const emre = {
  ...
  age: 2037 - 1998, // so tihs object called has 5 properties: property firstname with value "Emre", 
...
//because we can acces data in arrays with only index numbers
//we should use arrays with more structured datas
//and objects with unstructured datas
//this is the only considiration to use arrays or object though
//it will be expanded in the future or will come with experince
//or better yet learn it yourself

//How To Get The Property From An Object

console.log(emre.lastName); // the dot(.) is actullay an operator that goes the object(emre)
//and retrives the value we wanted using the keyword 

//we can use the square brackets instead of the dot but we need to write the keyword as a string
console.log(emre["lastName"]);
//biggest difference betwwen dot and bracket notation is:
...
const interestedIn = prompt("What do you want to know about Emre? Choose between firstName, lastName, job, age, and friends");

console.log(emre.interestedIn); // this will not work and give undefiden as a value
//reason this gives undefined is because when JavaScript tries to acces a property that doesn't exist it gives undefined

//we can use bracket notation for this

if (emre[interestedIn]) {
  console.log(emre[interestedIn]);
} else { console.log("Wrong request! Choose between firstName, lastName, job, age, and friends") }
...
console.log(`${emre.firstName} has ${emre.friends.length} friends and his best friend is ${emre.friends[0]}`);
...
const emreObjectMethod = {
  ...
  //any funciton that is attached to an object is a method
  //we can not use a funciton declaration
  ...
  //we can use this keyword for using the varibles that are in the object
  ...
  //let's say that the computaion we are doing is an intensive computation
  ...
  //and then we can accces that value later saving us computation time!
  ...
  getSummary: function () {
    ...
    return `${emreObjectMethod.firstName} is ${emre.age} years old ${emreObjectMethod.job} and ${this.hasDriversLicence ? "has a drivers licence" : "doesn't have a drivers licence"}`;
  }

};

console.log(emreObjectMethod.calcAge());
//we can acces this with bracket notation as well
console.log(emreObjectMethod["calcAge"]());
...
//Challenge
//Emre is a 39 years old student(lol), and he has a drivers licence
console.log(emreObjectMethod.getSummary());
console.log(emreObjectMethod);

//FOR LOOP
// for loop keeps running while the condition is true
//first we define a variable and then we create a contion such as rep<=10
//we need to use let keyword because with evert iteration of the loop the varable updates
//if the condition is true then for loop continous to run the code inside the loop body
...
//break is used for completely teminate the loop
