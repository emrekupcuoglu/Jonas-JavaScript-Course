...
const bookings = [];
//to set default parameters in ES6 we can write the inside the paranthesis
...
//we can't skip parameters when calling funcitons let's say we want to skip the numPassangers parameter
...
//object's memory adress is passed and that reference is still a value
...
const checkIn = function (flightNum, passenger) {
  //chancing the value of an arguments is usually a bad practice, this for learning purpouses
  flightNum = "LH999";
...
//we need to be careful with this behaveior and always keep it in mind
//because of the way objects behave when passed in to functions, this can have unforseeable concequences in large code bases
...
//The fact that JavaScript has functions as first clas citizens makes it possible to write higher-order functions
//*Higher-Order Functions are:
//A funciton that recives another function as an argumnent or returns a new function, or both
// btnClose.addEventListener("click", greet) is a higher order function beacuse it recives a function as its second argument
//and we usually say that the functions that is passed in is a //?callback funtion
...
//firsclass functions is a feature that programming languages can have all it means is that all functions are values
//there are no first class functions in practice its all concept
//there are higher order funcitons in practice however, because the language supports the first class functions
//it is asubtle difference but a difference nonetheless
...
const transformer = function (str, fn) {
  ...
  //here the paremter fn is a function so we can use it as a normal function
...
//we call the functions in the arguments callback functions, beacuse we don't call them but JavaScript does
...
//Another example
//Just like the transformer function we pass in a callback funtion to the addEventListener,
...
//and it calls the callback funtion high5 and it prints "High five!" to the console for 4 times in this case
["Emre", "Buğra", "Yaşar", "Ummahan"].forEach(high5);

//*The Advantages of Callback Funtions
//First big advantege is it makes it easy to split our code into more usable and interconnected parts
...
//Abstarciton basically means that we hide the detail of some code implementation because we don't really care about all that detail
//and this allow us to think about problems in a higher more abstarct level thats why it is called abstraction
//The transformer function doesn't carea t all how the string is transformed, it doesn't care about that detail
//All it does want to do is to trtansform a string but it doesn't care how it does it
//We could of write the oneWord or the upperFristWord funciton directly in the transformer that would of worked the same
//but instead we abstracted this code away into other functions.
//So we created a new level of abstraction and y doing this
//the transformer function is only concerner with transforming the input string itself but no matter how the actual transforming itself actually works
...
//the greet funciton returns a new function and we store that function in a new variable called greeterHey
...
//greet("Hello") is now a function. All of it is a function with the paranthesis and the "Hello" included
...
//so how do we fix the problem in other words how do we tell the JavaScipt we want to create a booking on the new eurowings airline
...
//Does NOT work
// book(23, "Ahmet");
//*but this throws an error because we are trying to acces the .this object with a normal function call and in a normal function call .this points to the window object and this throws an error. So we need another way to handle this.
...
//?Apply Method
//Apply essentially does the same thing, the only difference is that apply does NOT recieve a list of arguments after the .this keyword,
...
//?Bind Method
//Just like the call and the apply moethod bind also allow us to manually set the .this keyword for any function call
//The difference is that bind does NOT immidiately call the function instead it returns a new function where the .this keyword is bound
//So it is set to whatever value we pased to the bind
...
//so essantially partial application means that part of the arguments of the original function are already applied (which means already set)
...
//we can't use call or apply because they call the function immidiately
...
//So we write null it can be any other value because nothing will happen with it but it's kind of a stardard to write null 
const addVAT = addTax.bind(null, 0.18);
//so this is essantially the same as wrting:
...
//This might not be immediatly useful but it will be in the future especially with async await
...
runOnce();
//This is NOT we want we want to execute a function immediatly and not even having to save it somewhere
...
//We just wrap the function in paranthesis
//And don't forget to call it with adding paranthesis at the end!
(function () {
...
//!CLOSURES
//!Check lecture 137 and it's assosiated pdf
...
//A closure simply happens automaticly in certain situations we just need to recognize those situations.
//We will create one of thoso situations so then we can take a look at a closure .
...
//Booker function has increased the passangerCount variable from the outside.
...
//SecureBooking function has alredy finished it's execution so it's execution context is no longer on the stack but still the inner booker funciton is still able to access the passangerCount variable that is inside the booker function that no longer should exist.
//This is made possible with a closure(This is really weird btw)

//Closure makes a function remember all the variables that existed at the functions birthplace essentially and this can NOT be simply expalined with the scope chain alone
//We also need to understand closures

//Thanks to closures a function has access to the variable enviroment(VE) of the execution context in which it was created
//*Closure: VE attached to the function, exactly as it was at the time and place the function was created
//Closure has a priority over the score chain: For example: if there weas a global passangerCount variable that was set to 10 it would still first use the one in the closure 

//We can take a look at the closure using the console.dir()
//when this  logs to the console take a look at the Scopes property
//!Scopes property is basically the variable enviroment(VE) of the booker function
...
//*And that is true even when the variable itself was technically NOT even defined inside that variable enviroment(VE) f was created in the global scope but because it was assigned in the g function it still closed over the variable enviroment of the g function, because of that even though the execution of the g function ended it can still able to access the a variable
...
boardPassangers(21, 3);
//After the boardPassengers function is called perGroup is assingned
...
//and immediately console.log(`Will start boarding in ${wait} seconds`); thil line of code is executed
//after a set amount of time, 3 seconds in this case, the callback funtion of the setTimeout functions is called
//The callback functions is executed completely independent of the boardingPassengers function
//but still the callback function was able to use all the variables that were in the variable enviviroment in which it was created
//This is a clear sign of a closure being created
//Only way for the callback funtion to have access to the variables of the boardingPAssengers function after it's execution is ended is with a closure
