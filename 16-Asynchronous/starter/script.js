...
//Synchronous code is execuded line by line
...
//Asynchronous code is executed ater a task that runs on the background finishes(e.g when a using the setTimeout task runs after the timer ends)
//Asynchronous code is non-blocking
//Execution doesn't wait for an asynchronous task to finish its work

//?AJAX Calls
//AJAX stands for Asynchronous JavaScirpt And XML
...
//"Online" APIs: Applications running on a server, taht recieves,request for data, and sends data back in response(online term is not real it is used here just for emphasising the meaning)  
...
//JSON is popular becasue it is just a JavaScirpt object converted to a string,
...
//This is the oldschool way of doing things
//But it is still important to learn it because you might need it in the future
//And to see how AJAX calls used to be handled with events and callback funtions
...
const getCountryAndNeighbour = function (country) {
  ...
  //CORS stands for Cross Origin Resouce Sharing

  //First argument we pass in is the type of the request(the type of the http request to get data is smiply "GET")
  ...
  //The reason why we can't store it in a varaible is
  ...
  //This is the asynchronous, non-blocking behvaiour
  ...
  request.addEventListener("load", function () {
    ...
    //We would have ended up with callbakcs inside callbacks inside calbacks inside callbacks... and so forth
    //We have a special name for that kind of structure and behaviour
    //it is called "Callback Hell" 
    //Callback hell is when we have alot of nested callbacks in order to execute asynchronous tasks in sequence
    //Infact this happens for all asynchronous tasks which are handled by callbacks NOT just AJAX calls
    ...
    //and very diffucult to understand and to reason about
    //And code that is hard to udnerstand and,
...
//The reason for that is the data arrives at a slighly different time each time we reload the page
...
const getCountryDataPromise = function (country) {
  ...
  fetch(`https://restcountries.com/v2/name/${country}`)
    //Callback function of the then method will recieve an argument when it is called by the JavaScript
    ...
    .then(function (response) {
      ...
      if (!response.ok) {
        ...
        //and terminated the function using the throw keyword similiar to the return keyword
      ...
      //Without this second clalback function there was an uncaught error
      //You can of see the uncaugh error in the console without the callback function 
      //However we would need to handle the error on the second fetch function as well
      //And that can get annoying but we can handle all the errors globally on a acentral place
      // function (err) {
      //   return alert(err)
    ...
    .then(function (data) {
      ...
      //We can return an array of promises and render them acoordingly
    ...
    //We can handle all the errors no maatter where they appear on the chain by adding the catch method
    ...
    .catch(err => {
      ...
      console.error(`${err} 🚩`);
      //!IMPORTANT
      //The error that is generated is a real JavaScirpt object
...
btn.addEventListener("click", function () {
  //When we write a unvalid country an error is thrown
  ...
  //Because there is no rejection the catch handler caan NOT pick up on this error
...
//When a neighbour country doesn''t exist this code rejects the promise and only the oorigina lcountry is displayed
...
//JavaScipt doesn't run it, it runs in the web APIs runtime
...
//Event loop has the exteremly important task of coordinating the the callstack and the callbacks in the callback queue
//The event loop essentially decides exactly when each callback is executed
//We can also say the event loop does the orchestration of the entire JavaScirpt runtime
//But this presents a problem for timers
//Callback quene is an ordered data structure like a list
...
//This makes the timers in the JavaScipt unreliable
//it only guarantees that the funciton will be execuuted after 5 seconds
...
//But he engine itselft simply executes whatever code it is given

//*DOM events also use the callback queue even though they are not asynchronous

//*Things work slighly different with promises
...
//And if there are calllbacks in the microtasks queue
...
//Promsies are speacial kinds of objects

//Promise constructor takes exactly one argument
//and that is the so called executor function.
//As soon as the Promise constructor runs it will automaticly execute the
...
const getPosition2 = function () {
  return new Promise(function (resolve, reject) {
    //If the getCurrentPosition function automaticly calls these callback functions
    //and if it also automaticly passes in the position
...
//We turn this function into an async function by adding the async keyword infront of the function
//When this function is done it automaticly returns a promise(more on that later)
...
const whereAmI = async function () {
  ...
  try {
    ...
    const resGeo = await fetch(`https://geocode.xyz/${lat},${lng}?geoit=json&auth=704785914225166312361x14413`);
    ...
    //We hadneld the error inside the catch block but
    ...
    //and to catch the error exrternally we need to thro the error
...
const tryCatch = function () {
  ...
  //catch block will have access to the whatever error occured in the try block
  catch (err) {
    //with this script no longer ends abruptly 
    //we can cathc the error and handle it accordingly
    console.error(err);
...
const async1 = function () {
  whereAmI()
    ...
    //the then() method is executed instead of the catch() methdod
    //this means that the fucntion the async function returns is fulfilled even though there is an error
...
//?Promise.race
//Promise.race, just like other combinators, recives an array of promises and returns a promise
//Promise returned by Promise.race is settled as soon as one of the input promises settles
//In Promise.race bacsically first settled promise wins the race
//IF the winning promise is a fulfilled promise then the
//fullfilment value of the whole Promise.race promise is going to
//be the fullfilment of the winning promise
...
//In the real worl Promise.race is very useful too prevent against never ending promises or  very long promises
...
//So we can create a timeout promise which automaticly rejects after a certain time has passed 
...
//It is similliar to the Promise.all in that it also returns an array of promises
//but Promise.all short-circuits as sooon as one promise rejects
...
//Promise.any is very similliar to Promise.race
//The difference is with Promise.any rejected promises are ignored
//Therefore result of Promise.any is always going to be a fulfiled promise unless all of them reject
