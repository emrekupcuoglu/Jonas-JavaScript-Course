...
//?SLICE METHOD
//We can use the slice method to exrtact a part of the array
...
//*We can aslo use the slice operator for creating a shallow copy
console.log(arr.slice());
//This is the same as using the spread operator
console.log([...arr]);
//Using the slice or the spread operator is a personal preferance
//*But the only time the slicemethod is needed when you want to chain multiple methods

//?SPLICE METHOD
//Splice method is similiar to the slice method
// The difference is this: splice method mutates the argument while the slice method doesn NOT
//Splice returns a new array similiar to the slice method but it mutates the original array so that it only has the elements before the specifiel parameter
//In the example below the returned array is ["c","d","e"] and the original array is mutated into ["a","b"]
console.log(arr.splice(2));
console.log(arr);
//In many cases we don't cara about the retuned array
...
//The second aprameter of the splice function is not the end parameter but the delete count
...
//With this elemnts at index 1 and 2 are removed
arr.splice(1, 2);
console.log(arr);

//?REVERSE
//init
//Reverse method  reverses the order of an array and it mutates the original arra
arr = ["a", "b", "c", "d", "e"];
...
//?CONCAT METHOD
//This method concatonets the arrays into a new one
...
//Once again it is a personal preferance to use which one you want
...
//?AT METHOD
//This method is a new method adden in ES2022

const dummyArr = [23, 11, 64];
//If we wanted to take one of the elements from an arraay
//let's say the first one we would traditionally do this
console.log(dummyArr[0]);
//Wtih the add method we can do the exact same thing using a method
console.log(dummyArr.at(0));
//This doesn't seem al too useful but there is aparticular case where it matters
...
//The new at method makes thies process easier
...
//or start counting from the end of an array you sprobably use the at method
...
//*At method also workds on strings
console.log("emre".at(-1));

//?LOOPING OVER ARRAYS: forEACH METHOD

//Positive values are deposits to a bank account and the negetives are withdrawals
const movements = [200, 450, -400, 3000, -650, -130, 70, 1300];
...
//forEach is ahigher order function taht takes a callback function
...
//in each iteration the callback function recieve the element as an argument(the parameter we specified as movement will get the element of that iteration as an argument )
...
//*It also passes in the current element, current index, and the entrie array we are looping over
//Therfore we can specify tham in the parameter list
console.warn("Start of the forEach loop");
...
//!When you need to break out of a loop you  must use a for of loop but other than that it comes down to preferance just like so many other thing is JavaScript
...
//When we call forEach on maps the paramters vary slightly
...
//The reason this is the way it is beacuse:the designers of this method wanted to maintain a consistency
//If they had omitted the key parameter, than it would have been a completely diifferent method seperate from the forEach method
//So this works this way because of consistency and it's not that hard to keep the sytax in mind
//And to avoid that confusion we can just write an underscore there
//The underscore in JavaScripot means a throwaway variable which is a variable that is completly unnecessary
...
//*Map method
//Map method is similiar to the forEach funtion taht we can use it to loop over arrays
//but it returns a new array. This new array will contain in each position results of appyling a callback funtion to the original array
...
//We can also use an arrow function as a callback funtion to simplify it as well
...
//This method also can have three parameters value, index, and the entire aray but there are rarely used except the value
//to filter the array w ereturn a boolean value
...
//We can also use it with arrow funcitons
const withdrawals = movements.filter(mov => mov < 0);
console.log("withdrawals", withdrawals);

//*Reduce method
//Reduce boilds ("reduces") all array elements wown to a single element(e.g. adding elements togerther)
//The first parameter of the reduce method is the acumulator second is the current element, third is the index, and the fourth is the entire array
//Accumulator is like a snowball that keeps acuumulating the value we ultimately return
...
const balance = movements.reduce(function (acc, current, i, arr) {
  ...
  //We need to return the accumulator becuase in each iteration accumulator is returned and updated
...
const max = movements.reduce((acc, cur) => cur > acc ? acc = cur : acc
  , movements[0]);
...
//Let's say that there is a mistake in the fliter method instead of returning deposits it returns withdrawals
//If we want to check the result of the filter method we need to check the array at the next method
//So if tehre is a problem with the array returned from filter and we want to check it we need to check it in the map method
//Because the resulting array from thefilter method is the input of the map method in this case
...
//*Find Method
//Find method finds first occurance of of an element that satisfies a specified condition
...
//!This example is a few lines below around line 570 because of initilization reasons, beginning of it is marked with a green text

//*FindIndex method
//This method is similiar to the indexOf method but with indexOf we can only check if an elements is inside the array
...
//These methods are similiar to the includes method but with includes we can only check for equality
...
//*Every Method
//Every method is similiar to the some method but it only returns true if all the elements in that array satisfy the specified condition
console.warn("every Method");
console.log(movements.every(mov => mov > 0));

//*Seperate Callbacks
//Until now we always srote the callback functinos inside the array methods but we can write the seperately somewhere else and call them as well
...
//Flat Method goes only one level deep when flatteing an array
...
//*flatMap Method
//It turns out mapping an array and the nflatting it is a fairly common operation
...
//For now we are simply going to use the JavaScirpts built in sort method
...
//callback function checks substracts b from a ----=> a-b
...
//and returning a negative bumber if a<b
...
//fill method is a bit similiar to the slice method
...
//We can also use fill method on other arrays it doesn NOT have to be an empty array
...
const random = Array.from({ length: 100 }, () => Math.trunc(Math.random() * 6) + 1);
...
//The reason we are using objects instead of maps is beacuse  we are pretending we are getting the dara from an API 
...
console.log("allBalance", overalBalance);
...
//But it is alot better to pass that data directly into the function
...
const displayMovements = function (movements, sort = false) {
  ...
  //This method is simlia to the textContent
  ...
  <div class="movements__row">
    ...
    //Wtih "afterbegin" each new element added is added before the old one
    //We can change this behaviour with "beforebegin", "afterend", "beforeend" 
...
const calcDisplaySummary = function (account) {
  const income = account.movements.filter(mov => mov > 0).reduce((acc, mov) => acc + mov);
...
btnLogin.addEventListener("click", function (e) {
  //In HTML default behaviour for a submit button is to refresh the page
  //Because this is a button in a from element we need to prevent it from its default behaviour
  e.preventDefault();
  // console.log("login");
...
labelBalance.addEventListener("click", function () {
  const el = Array.from(document.querySelectorAll(".movements__value"), el => el.textContent.replace("€", ""));
  console.log("el", el);

  //We can also use the spread operator
  //But then we would have to do the mapping seperately
...
//!Which Array to Use
//Check out lecture 165 and pdf for more inforamtion



//?Array Methods Practice

//*Let's get the sum of all the deposits that are deposited int othe bank
...
//This gives us a big array with subarrays that contain the movements we need all of them in the same array so we flatten the array
...
//We can also do this with a reduce method is a bit more comlex but it shows the power of the reduce method
...
//++ operator doesn increment the value but returns the old value
let a = 10;
...
const { deposits: numDeposits4, withdrawals: numWithdrawals } = accounts
  ...
  .reduce((sums, cur) => {
    ...
    //Using the brackets notation insted of the dot notation
...
const convertTitleCase = function (title) {
  ...
  const converted = title
    .toLowerCase()
    .split(" ")
    .map(word => exceptions.includes(word) ? word : capitilize(word)).join(" ");
  return capitilize(converted);
};
