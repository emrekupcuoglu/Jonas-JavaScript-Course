...
//Code below is unefficient because we are attaching the same event handler to all html elements
...
//   e.preventDefault();
//   console.log("LINK");
//We use getAttriubet and not the propery selector, because we want the relative URL not the absolute URL
...
//!Event delegation is especially important when working with elements that are NOT yer on the page. Like buttons taht are added dynamicly when using the page
...
document.querySelector(".nav__links").addEventListener("click", function (e) {
  ...
  //We need to match the elements and only handle the events taht match
...
tabsContainer.addEventListener("click", function (e) {
  ...
  //We can use the closest methdod for that
  ...
  //We can use optionel chaining or a guard clause to fix this
  //So if the clicked is not a truhty value we exit the function without running the code below it
...
const handleHover = function (e,) {
  ...
  //But for learning purpouses this is done like this
...
//mouseover and mouseenter are similiar but mouseenter does not bubble
...
//Infact it is impossible to pass an argument to an event handler function
...
const obsCallback = function (entries, observer) {
  ...
  //So in this example whenever the target is intersecting the viewport at 10%(viewport beacuse we set the root to null)
  //then the callback funtion will be called. It doesn't matter that we scroll up or down it will be called.
...
const obsOptions = {
  ...
  //root element is the element that we want the target to intesect
...
const stickyNav = function (entries, observer) {
  ...
  //if it is intrersecting with the viewport we make it unsticky
...
const revealSection = function (entries, observer) {
  ...
  //But right now we are observing all the secitns with only one observer
  //So we need to make the distinction between sections
  //And we use the target property for that
  //We need to use this if statement because when we load the page an intersection event happens automaticly
  //with the target of seciton--1 so the first section is not animated
  ...
  //We don't need the onserver after making the sections visible so we can remove the observer to not waste resources
...
const loadImg = function (entries, observer) {
  ...
  //beacuse we lift the blur on the image as soon as possible
  //instead we need to wait for the image to load then remove the blur
  // entry.target.classList.remove("lazy-img");

  //Remove the blur on load
  //This is a bit tricky beacuse replacing of the src with the data-src happens bedhind the scenes
  //JavaScirpt finds the new image that it should load and dislay it behind the scenes
...
const slider = function () {
  ...
  dotContainer.addEventListener("click", function (e) {
    if (e.target.classList.contains("dots__dot")) {
      ...
      //Code below return a string in this it doesn't matter beacsue JavaScipts type coercion works correctly for the operations in the goToSlide function
...
//Just typing documenet is not enough to select the document element
//Because document is not a DOM element
//so for example if we want t oapply css styles to the entire page we need to select the document
...
//We need to use query selector or other methods to select toher html elements
...
//That means if the DOM changes then this selection is also immediatly updated automaticly
...
//We can create HTML elements using insertAdjacentHTML this was covered in the previous secitons
...
document.querySelector(".btn--close-cookie").addEventListener("click", function () {
  ...
  //beacuse this was the old way of doing it and many people also don't know about the remove method 
...
//The reason we are setting the width to larger than 100% is beacuse we want the empty seciton on sides to be covered as well
...
//Let's say that we wanted to incerase the height of the message banner by 40px
...
//And in JavaSciprt the root is equivalent to the document
...
//*Attributes
//We can access the attribues by using the names of the attributes
const logo = document.querySelector(".nav__logo");
//This workds only on standart attributes
//like alt or src for images
//If we specify these standart attributes in HTML
//than the JavaScipt will automaticly create these properties on the Object
...
//*But if we add some other property that is not a standart property
//JavaSciprt will NOT automaticly create a property on the object
//This return undefined because this is NOT a standart property that is expected to be on images
...
//We can also use the setAttribute to set non-standart attributes
...
//If we want to get the realative url we can use the getAttribute
console.log(logo.getAttribute("src"));

//!Same is true for href attribute on links as well

const link = document.querySelector(".twitter-link");
//These two gives the same results beacuse
...
//We use data attributes quite alot when we work with the UI
...
//Signal means something has happened like a click, the mouse moving or the user triggering the full secreen mode etc.
...
//! 2 Ways the addEventListener is better thjan this approach
//1. Add event listener allows us to ad multiple event listenres to the same event
...
const alerH1 = function (e) {
  ...
  //But this doesn't have to happend inside the event listener callback function
...
//First we add ana ttribute to the HTMl element that we want to add the event listener to(onclick in this case)
...
//it will pass through every parent element of the tager element
...
//Event bubble up from the taget to the document root
//just like the capturing phase event pases through all the parent elements 
...
//then we would get the exact same alert window for the seciton element as well
...
//*.target is where thevent originated NOT wher eit is handled it is not hte palce where the handler is attached
...
//BUt in genereal it is not a good idea to stop propagation of events
//e.stopPropagation();
// });
...
//Default behaviour is bubbling and use that unless capturing is needed
...
//This code slecdts all the elements with the class hightlight that are the childeren of the h1 element
//This works no matter how deep the child elements are inside the h1 element
//In this case highlight class is the direct childeren of the h1 element
//But if they were not it would go dows as deep as neccessary into the DOM tree
//And if there were other elemens with the class highlight
...
//Sometimes we only want the direct childeren
//For that we use:
console.log("childNodes", h1.childNodes);

//We can use this to get the HTML colleciton
//This only works for direct childeren
...
//In JavaScirpt we can only acces the direct siblings
...
//We can move upto the parent element and read all the childeren from there
...
//?Life Cyle of DOM Events

//DOM Content Loaded
//This event is fired by the document as soon as the HTML is completely parsed
//Alos all scripts must be downloaded and executed before this event fires
//We can listen to this event since it happens on the document

//This event doesn't wait for images or other exernal resources to load
...
//No. Beacuse the <script> tag in HTML links the HTML and JavaScirpt and put the <script> tag at the end of our JavaScirpt so we don't need to wrap it
document.addEventListener("DOMContentLoaded", function (e) {
  console.log("HTML is pardes and DOM tree is built!", e);
...
//This event is created immediatly before a user is about to lave the page
...
// window.addEventListener("beforeunload", function (e) {
  //In some browsers to make tihs work we need to call prevent default
  //It is no neccessary on chrome
  // e.preventDefault();
  // console.log(e);
  ...
  //But many people started to abuse this, so now we can olny see a generic message
...
//?Async 
//When we use the async keyword as an attribute in the <script> tag we can fetch the script asyncronously
//So script is fetched as the HTML is parsed,
//but when the script is fetched HTML sopts parsing and script gets executed
//This is better and faster than the default behaviour but we can improve it with defering it

//?Defer
//With defer script is fetched asyncronously,
//but the execution of the scripts is defered until the the end of the HTML parsing
//So in practice loading time is similiar to the async but the key difference is this
